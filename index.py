...
def calculate_parts(part_lengths,number_of_tubes, tube_length):
    prob = LpProblem("Tube Cutting Problem", LpMinimize)
    x = LpVariable.dicts("Number_of_Parts", part_lengths.keys(), 3, None, cat='Integer')
...
# Streamlit UI
st.title("Tube Parts Calculator")
...
if submit_button and st.session_state.part_lengths:
    max_parts, total_waste,x = calculate_parts(st.session_state.part_lengths, no_of_tubes, tube_length)
